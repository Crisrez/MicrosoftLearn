...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            int[] faltantes = new int[maxPets];
            
            Console.WriteLine();
            Console.WriteLine("Las siguietnes mascotas les faltan las edades y descripciones fisicas: ");
            Console.WriteLine();

            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0].Length > 6 && (ourAnimals[i, 2].Contains('?') || ourAnimals[i, 4].Length < 25))
                {
                    for (int j = 0; j < faltantes.Length; j++)
                    {
                        if (faltantes[j] == 0)
                        {
                            faltantes[j] = i;
